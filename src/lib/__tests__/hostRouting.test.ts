...
import { CHAT_HOST, SITE_HOST, LEGACY_HOSTS, PREVIEW_CHAT_HOST, PREVIEW_SITE_HOST, routeByHost } from '../hostRouting';
...
describe('routeByHost', () => {
  it('permanently forwards every deprecated host to its replacement, path and query preserved', () => {
    ...
    expect(at('hire.adam.matthewsteinberger.com', '/hire-me')).toEqual({ kind: 'redirect', status: 301, location: `https://${SITE_HOST}/hire-me` });
    ...
    expect(at('WWW.MATTHEWSTEINBERGER.COM')).toEqual({ kind: 'redirect', status: 301, location: `https://${SITE_HOST}/` });
  });
  ...
  it('keeps API, Next internals, and file assets on the chat host, and sends pages to the site host', () => {
    ...
    expect(at(CHAT_HOST, '/story')).toEqual({ kind: 'redirect', status: 308, location: `https://${SITE_HOST}/story` });
    expect(at(CHAT_HOST, '/work/x?y=2')).toEqual({ kind: 'redirect', status: 308, location: `https://${SITE_HOST}/work/x?y=2` });
  });
  ...
  it('routes the preview pair the same way, and never crosses into production', () => {
    ...
    expect(at(PREVIEW_CHAT_HOST, '/story?y=1')).toEqual({ kind: 'redirect', status: 308, location: `https://${PREVIEW_SITE_HOST}/story?y=1` });
    expect(at(PREVIEW_CHAT_HOST, '/api/ask')).toEqual({ kind: 'pass' });
    expect(at(PREVIEW_SITE_HOST, '/chat')).toEqual({ kind: 'redirect', status: 308, location: `https://${PREVIEW_CHAT_HOST}/` });
    expect(at(PREVIEW_SITE_HOST, '/hire-me')).toEqual({ kind: 'pass' });
  });
  ...
  it('moves /chat on the site host to the chat host and leaves everything else alone', () => {
    expect(at(SITE_HOST, '/chat')).toEqual({ kind: 'redirect', status: 308, location: `https://${CHAT_HOST}/` });
    expect(at(SITE_HOST, '/hire-me')).toEqual({ kind: 'pass' });
